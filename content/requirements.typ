...
*FR3 -- Manage Courses and Members.* Instructors shall create and manage courses. Each course has a name and a semester identifier. Instructors shall add course members individually or through a CSV import, and remove members individually. Each member holds a role -- either STUDENT or INSTRUCTOR -- within that course. A user may hold different roles in different courses simultaneously: a teaching assistant, for example, may act as an INSTRUCTOR in one course while enrolled as a STUDENT in another. Each role grants a distinct set of permissions and a distinct interface view for that course. The system recalculates a user's global role when a membership is added or removed; a dedicated recalculate endpoint is available for role updates.
