...
In the second step, the client submits the code via `POST /api/auth/verify-code`. The service looks up the pending entry, confirms the code matches and has not expired, and creates the `AppUser` record in the database. Passwords are hashed using BCrypt at strength 10 before storage, an adaptive hashing scheme designed to resist brute-force attacks @provos1999bcrypt. The service removes the pending entry from memory immediately to prevent reuse, and the `AuthController` establishes an HTTP session so the user is logged in upon registration without a separate login step.

The registration flow handles a second case: users whom an instructor adds to a course before those users have registered. In this case, a partial `AppUser` record with no password hash already exists; `verifyCodeAndCreateAccount` detects it and completes the account in place rather than creating a duplicate. The mechanism is described in full in @sec-course-member-management.

#diagram(
  image("/figures/activity_diagram_paperr.drawio.png", width: 120%),
...
The password reset flow consists of three steps. The client first calls `POST /api/auth/forgot-password` with an email address. The endpoint always returns HTTP 200 regardless of whether the email belongs to a registered account, preventing email enumeration. When an account exists, the service generates a UUID#footnote[RFC 4122 – A Universally Unique Identifier (UUID) URN Namespace: https://www.rfc-editor.org/rfc/rfc4122] reset token, stores it in memory with a 15-minute expiry, and dispatches a reset link by email. The client then optionally calls `GET /api/auth/validate-reset-token` to confirm token validity before presenting the reset form. Finally, `POST /api/auth/reset-password` accepts the token and a new password, validates both, hashes the new password with BCrypt, and consumes the token so it cannot be reused.
...
For CSV import, `POST /api/courses/{courseId}/members/import` accepts a CSV file with columns `first_name`, `last_name`, `tum_id`, and `tum_email`. The backend detects the delimiter (comma or semicolon), applies the same two-case logic to each row, and returns a per-row success and error report. @fig-csv-format illustrates the accepted file formats.

Each row undergoes server-side validation against several constraints: required fields (first name, last name, email) must not be empty; the email must match a TUM domain (`@tum.de` or `@mytum.de`); the TUM ID, if provided, must follow the expected format (consonant-vowel-2digits-consonant-vowel-consonant, e.g., `ge57yef`); within the same import, email addresses and TUM IDs must not be duplicated; and the user must not already be a member of the course. Rows that pass all validations are imported; rows that fail include a specific error reason in the report.
...
#diagram(
  ...
  caption: "CSV member import interface displaying validation results. The backend returns per-row success and error information which the interface presents clearly to the instructor: a summary line showing the number of successful imports and errors, followed by a detailed list of validation errors with line numbers and specific reasons, and a table of successfully imported participants with their assigned roles.",
  short-caption: "CSV member import interface with validation results",
) <fig-csv-import-ui>

Beyond adding members, the backend exposes endpoints for updating and removing memberships. `PUT /api/courses/{courseId}/members/{userId}` and `PATCH /api/courses/{courseId}/members/{userId}` both update the role a user holds in a course. `DELETE /api/courses/{courseId}/members/{userId}` removes the membership record and recalculates the user's `globalRole`. How `globalRole` is derived from per-course memberships and why this matters for user permissions is explained in the Role Management section below.

=== Role Management

TUMPeer derives each user's effective interface from the roles they hold across all their course memberships. Three cases arise: STUDENT memberships only yields the student view; INSTRUCTOR memberships only yields the instructor view; holding both yields a toggle between the two views. Figure~@fig-switch-button shows the toggle as it appears in the student dashboard for a user who holds an instructor role in another course.

#diagram(
  image("/figures/switch_button_updated.png", width: 100%),
  caption: "Student dashboard for a user who holds an instructor role in another course. The \"Switch to Instructor\" button in the navigation bar lets the user toggle to the instructor view; the backend determines which button to expose based on the roles returned by GET /api/auth/me.",
  short-caption: "Role toggle button in the student dashboard",
) <fig-switch-button>

The backend implements this through two fields returned by `GET /api/auth/me`. The first is `globalRole` on `AppUser`, a single enum value (`STUDENT` or `INSTRUCTOR`) that records the highest role the user holds across all course memberships. `UserService.recalculateGlobalRole()` keeps this field in sync: it queries all `CourseMember` records for the user, sets `globalRole` to INSTRUCTOR if any membership carries that role, and sets it to STUDENT otherwise. Storing `globalRole` as a denormalised column avoids recomputing it from course memberships on every request; the trade-off is that it must be explicitly recalculated whenever course memberships change. The second field is `courseRoles`, a deduplicated list of all distinct role values the user holds across their course memberships. The endpoint collects every `CourseMember` record for the user, extracts the role from each, removes duplicates, and returns the result (for example, `["STUDENT", "INSTRUCTOR"]` for a user who holds both roles). The frontend reads `courseRoles` to decide which views to render: a list containing only `"STUDENT"` shows the student view; only `"INSTRUCTOR"` shows the instructor view; both values enable a toggle between the two views.
...
The allocation requirements — no self-review, equal workload distribution, deterministic results — are specified in FR6. Ensuring fair and balanced workload distribution is important so that all students are treated equally in the review process. The algorithm that satisfies these requirements runs in `ReviewService.allocateReviewsForAssignment()`. It takes the list of submissions -- and therefore the list of reviewer candidates, one per submission -- and that configured number of reviews per submission.
...
The algorithm runs in $O(n^2 log n)$ time, where $n$ is the number of submitting students. The dominant cost is the candidate sort executed once per submission: sorting $n - 1$ candidates takes $O(n log n)$, repeated $n$ times yields $O(n^2 log n)$ overall. At typical course enrolment of 20 to 300 students, allocation is expected to be fast in practice and executes exactly once per assignment, so the quadratic factor carries no significant practical cost at this scale. A priority-queue-based variant could reduce the overall complexity to $O(n log n)$ by maintaining a sorted structure of reviewer loads across iterations, but at higher implementation complexity for negligible benefit at this scale. Random allocation was rejected because it is non-deterministic and cannot guarantee equal load distribution; bipartite matching would guarantee an optimal assignment, but the improvement over the greedy approach is negligible at typical course enrolment scales.
...
#diagram(
  image("/figures/assignment_status_table_tum 2 (2).png", width: 120%),
  caption: "Status implementation diagram for TUMPeer. The timeline shows all five lifecycle phases with submission statuses visible to students, review statuses visible to reviewers, and the assignment period label visible to the instructor.",
...
Phase 5 formally begins when `reviewEnd` passes, but no change is immediately visible to students at that moment. The scheduler transitions every `ReviewAssignment` with status `"READY FOR REVIEW"` to `"NO REVIEW SUBMITTED"`. For assignments with status `"DRAFT REVIEW"`, it calls `clearDraftReviewAnswers()`, which clears the saved scores and comment text and transitions the status to `"NO REVIEW SUBMITTED"`. Only reviews with status `"REVIEW SUBMITTED"` contribute to grade calculation; incomplete and overdue reviews are excluded. The assignment period remains `"Review Period"` on the instructor dashboard. The instructor may still enter or update instructor grades for any submission before triggering grade release.

The first change visible to students occurs only when the instructor explicitly triggers grade release via `POST /api/assignments/{assignmentId}/release-grades`. The endpoint computes the final score for every submission using the peer review scores and the instructor score, applies the ±20pp outlier detection rule described in the following section, transitions every `UNDER_REVIEW` submission to `GRADED`, and sets `assignment.resultsReleaseAt` to the current time. The assignment period advances to `"Grades Released"`. Students with status `GRADED` can now view their final score and the reviews they received. `NO_SUBMISSION` records are not advanced to `GRADED` and contribute a score of zero to the student's course statistics. 
=== Scheduler

Why the scheduler uses a polling approach and what the 60-second interval means in practice is discussed in @sec-control-flow. One important property of the implementation is that running the scheduler multiple times has no unintended effects: before updating a record, the scheduler checks whether it is already in the target status and skips it if so. The scheduler also reads the current time in the Europe/Berlin timezone on every run, so status transitions happen at the correct local time even when the clock shifts for daylight saving.

== Grading System

Every submission is evaluated using the same rubric by the assigned peer reviewers and, optionally, by the instructor. Each evaluation produces a percentage score computed as `(totalPointsAwarded / totalPointsMax) × 100`. The backend tracks which `ReviewAssignment` records belong to the instructor by checking course membership, and keeps the two sets of scores separate for outlier detection. The instructor may submit their rubric evaluation at any point after the review period starts; peer reviewers work within the review window as detailed in the section on status transitions.

=== Peer Grade Preview

Before triggering grade release, the instructor sees a summary table showing each student's average peer score and instructor score. The backend provides these values through the submission review summary endpoint; the frontend uses them to display a preview of the likely final score. The definitive final grade — incorporating the instructor score and applying the ±20pp outlier detection rule — is computed and stored by the release-grades endpoint, which also makes the results visible to students.

=== Bulk Grade Release with Outlier Detection

The endpoint `POST /api/assignments/{assignmentId}/release-grades` processes all submissions in a single transactional operation and computes the definitive final grade for each. For each submission, the service collects all submitted peer reviews into two sets. `allPeerScores` contains the percentage score from every `ReviewAssignment` with status `REVIEW SUBMITTED`, including the instructor's when they graded. `studentPeerScores` is the subset restricted to non-instructor reviewers and is used exclusively for the outlier check. The instructor grades a submission through the same rubric form as student reviewers; this single action creates a `ReviewAssignment` for the instructor and simultaneously stores the computed percentage to `SubmissionGrade.instructorScore`, which the outlier check uses as its reference value. Four cases determine the final score. In statistical terms, outliers are data points that differ substantially from the majority of observations and may indicate abnormal or unreliable behaviour @aggarwal2017outlier. In the context of peer assessment, such deviations can signal inconsistent or unreliable evaluations rather than true differences in submission quality.
...
The ±20pp threshold uses a strict inequality, is checked per individual score, and applies only to student reviewers, so an instructor's own peer review submission does not trigger the outlier rule against their own direct grade. The threshold was set as a project requirement by the supervising instructor. The threshold is a configurable starting point — a statistical replacement such as a z-score or interquartile-range-based rule is identified as future work.

#figure(
  image("/figures/grades_view.png", width: 100%),
