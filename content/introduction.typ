...
Peer review is an established component of university education. When students evaluate each other's work using structured criteria, they develop evaluative judgment, encounter approaches to the same problem that differ from their own, and receive feedback that extends beyond what a single instructor can provide for an entire cohort. At scale, however, managing peer review manually becomes impractical. Instructors must assign reviewers fairly, enforce submission and review deadlines, collect structured feedback from hundreds of students, and reconcile multiple peer evaluations into a single final grade — tasks that grow linearly with course enrolment and consume time that would otherwise go to teaching.

At the Technical University of Munich (TUM), large introductory and project-based courses regularly enrol several hundred students. Existing learning management systems offer basic peer review capabilities but lack the automation, rubric-based grade computation, and TUM-specific institutional constraints that large-course operation requires. TUMPeer is a dedicated, automated peer review platform built to fill this gap. It manages the complete peer review lifecycle through a REST API backed by a PostgreSQL database: from user registration and course setup, through file submission and automatic reviewer allocation, to rubric-based grading with outlier detection and anonymised result release. To provide an initial overview of the system from a user perspective, Figures 1 and 2 show the student and instructor dashboards.

#diagram(
  image("/figures/student_view_final.png", width: 100%),
  caption: "Overview of the TUMPeer student dashboard. Submissions and review tasks across multiple courses are presented as cards with their current status. This screenshot is included for orientation only; the backend implementation is the subject of this thesis.",
  short-caption: "TUMPeer student dashboard (overview)",
)

#diagram(
  image("/figures/instructor_view.png", width: 100%),
  caption: "Overview of the TUMPeer instructor dashboard. This screenshot is included for orientation only; the backend implementation is the subject of this thesis.",
  short-caption: "TUMPeer instructor dashboard (overview)",
)

== Problem

At TUM, courses that include peer review currently rely on a patchwork of manual steps and disconnected tools. Students upload their submissions to Moodle or Artemis; instructors then anonymise the files by hand, pair reviewers with submissions manually, and distribute the anonymised work through ad-hoc channels. After the review window closes, instructors collect rubric scores and written comments in spreadsheets and must again manually strip reviewer identities from the collated data before sharing results with students. Each of these steps is time-consuming, difficult to audit, and fragile at scale: manual anonymisation introduces errors, reviewer allocation lacks consistency, and merging multiple rubric evaluations into a single final score is tedious. The overhead is significant enough that some instructors resign from peer assessment altogether, depriving students of its pedagogical benefits.

Running this workflow without dedicated backend infrastructure creates several further concrete problems. Assigning reviewers fairly — ensuring each reviewer receives the same number of tasks and that no student reviews their own work — requires a combinatorial allocation step that cannot be reliably performed by hand for large cohorts. Enforcing submission and review deadlines requires continuous monitoring; without automation, late submissions or missing reviews are easy to overlook. Detecting outlier peer scores and correcting the final grade accordingly requires explicit business logic that spreadsheet-based approaches cannot provide. And because data is scattered across multiple tools, it is impossible to guarantee anonymity, fairness, and consistency simultaneously without continuous manual oversight.

TUM courses impose additional institutional constraints that generic platforms do not accommodate. Registration must be restricted to `@tum.de` and `@mytum.de` email domains. Reviewer identities must remain hidden from submission authors, and submission-author identities must remain hidden from reviewers, throughout the entire workflow. Grade computation must follow a defined outlier-detection rule: when any peer score deviates by more than 20 percentage points from the instructor score, the instructor score overrides the peer average.
